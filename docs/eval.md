...
| Gate latency | 113.0 ms | 0.73 ms | 155× faster |
...
| Stage | Budget | Measured | |
|---|---|---|---|
| Prefill (643 tokens) | 150 ms | 445 ms | 3.0× over |
| Generate (67 tokens) | 500 ms | 7035 ms | 14.1× over |
| **Turn total** | **650 ms** | **7480 ms** | |

Decode runs at **105 ms/token**; transformers'
own `generate` manages 119 ms/token on the same
